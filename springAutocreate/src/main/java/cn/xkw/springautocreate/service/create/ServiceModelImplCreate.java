 ...
 * @ProjectName: srbank
 * @Version 1.0
 * @Description: DataResponse创建类
 */
@Service
@ListManage
public class ServiceModelImplCreate implements BaseSrBankCreate {
    ...
    public Boolean create(Model model) {
        try {
            ...
            Template template = cfg.getTemplate(CommonData.TEMPLATE_FILE_SERVICE_Model_IMPL_NAME);

            // 解析数据
            // 包名
            model.setPackageName(CommonData.DATA_API_PACKAGE_NAME + model.getCentName().split("-")[1] + ".per.api.data.response");
            ...
            File moduleFile = new File(CommonData.SRBANK_WRITE_FILE_PATH + model.getClassName() + "/" + model.getClassName() + "ServiceImpl.java");
