 ...
 * @ProjectName: srbank
 * @Version 1.0
 * @Description: DataRequest创建类
 */
@Service
@ListManage
public class DataRequestCreate implements BaseSrBankCreate {

    private final Logger LOGGER = LoggerFactory.getLogger(DataRequestCreate.class);
    ...
    public Boolean create(Model model) {
        try {
            ...
            Template template = cfg.getTemplate(CommonData.TEMPLATE_FILE_DATA_REUEST_NAME);

            // 解析数据
            // 包名
            model.setPackageName(CommonData.DATA_API_PACKAGE_NAME + model.getCentName().split("-")[1] + ".per.api.data.request");
            ...
            File moduleFile = new File(CommonData.SRBANK_WRITE_FILE_PATH + model.getClassName() + "/" + model.getClassName() + "Request.java");
            ...
            LOGGER.info("--- data request处理异常 ---", e);
            return false;
        }
        return true;
    }
